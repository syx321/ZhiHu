...
enum TimeFormat:String {
    ...
    case YYYYMMDDHH = "YYYYMMdd HH"
    case YYYYMMDDHHMM = "YYYYMMdd HH:mm"
    case YYYYMMDDHHMMSS = "YYYYMMdd HH:mm:ss"
    case YYYYMMDDHHMMSSsss = "YYYYMMdd HH:mm:ss.SSS"
}

extension Date {
    
    ///获取当前时间
    static func getCurrentTimeString(timeFormat: TimeFormat) -> String {
    ...
    static func getCurrentPastTimeString(day: Int,timeFormat:TimeFormat) -> String {
...
extension Date {
    ///获取当前日期Date
    static func getCurrentTime(timeFormat: TimeFormat) -> Date {
        let formatter = DateFormatter()
        formatter.dateFormat = timeFormat.rawValue
        let timezone = TimeZone.init(identifier: "Asia/Shanghai")
        formatter.timeZone = timezone
        let dateTime = formatter.defaultDate
        return dateTime!
    }
    ...
    ///   - timeFormat: 时间格式
    ///   - timeString: 要转的字符串时间
    static func timeToTimestamp(timeFormat: TimeFormat,timeString: String) -> String {
        let formatter = DateFormatter()
    ...
    ///   - timeFormat: 时间格式
    ///   - dateString: 当前时间
    static func getLastDay(timeFormat:TimeFormat,dateString: String) -> String {
    ...
    ///   - timeFormat: 时间格式
    ///   - dateString: 当前时间
    static func getNextDay(timeFormat: TimeFormat,dateString: String) -> String {
        let lastDay = NSDate.init(timeInterval: 24 * 60 * 60, since: dateFromString(timeFormat: timeFormat, date: dateString) as Date)
        let formatter = DateFormatter()
        formatter.dateFormat = timeFormat.rawValue
        let strDate = formatter.string(from: lastDay as Date)
    ...
    /// 获取当前时间是星期几
    static func getNowWeekday() -> String {
        let calendar:Calendar = Calendar(identifier: .gregorian)
    ...
    ///   - timeFormat: 时间类型
    ///   - timeString: 时间戳
    static func getTimeFromTimestamp(timeFormat:TimeFormat,timeString:String) -> String {
    ...
    /// - Parameters:
    ///   - timeFormat: 时间类型
    static func getNowTimeTimestamp(timeFormat: TimeFormat) -> String {
        let formatter = DateFormatter()
        formatter.dateStyle = .medium
        formatter.timeStyle = .short
        formatter.dateFormat = timeFormat.rawValue
    ...
    ///   - endTime: 结束时间
    ///   - starTime: 开始时间
    static func acquisitionTimeDifference(timeFormat: TimeFormat,endTime: String,starTime: String) -> NSInteger {
        let formatter = DateFormatter()
        formatter.dateFormat = timeFormat.rawValue
    ...
    ///   - date1: 时间1
    ///   - date2: 时间2
    static func compareDate(timeFormat:TimeFormat,date1:String,date2:String) -> Int {
        var ci = Int()
        let formatter = DateFormatter()
        formatter.dateFormat = timeFormat.rawValue
        var dt1 = NSDate.init()
    ...
    /// 获取过去某个时间
    /// - Parameter timeFormat: 时间格式
    static func getCurrentPastTime(day: Int, timeFormat:TimeFormat) -> String {
        let mydate = NSDate.init()
        let formatter = DateFormatter()
        formatter.dateFormat = timeFormat.rawValue
        let calendar:Calendar = Calendar(identifier: .gregorian)
    ...
    /// - Parameters:
    ///   - timeFormat: 时间格式
    static func getTomorrowDay(timeFormat:TimeFormat) -> String {
        let calendar:Calendar = Calendar(identifier: .gregorian)
        var comps:DateComponents = DateComponents()
        comps = calendar.dateComponents([.year,.month,.day,.weekday,.hour,.minute,.second], from: Date())
